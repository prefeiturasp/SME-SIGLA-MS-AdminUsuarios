...
def test_email_service_enviar_email_and_reset(monkeypatch: Any, settings: Any) -> None:
    ...
    EmailService.enviar_email(subject='Assunto', template_name='x.html', context={'k': 'v'}, recipients=['dest@example.com'])
    user = User.objects.create_user(username='alice', first_name='Alice')
    monkeypatch.setattr('usuarios.services.email.TokenService.gerar_token_para_reset', lambda *_args, **_kwargs: {'uid': 'uid1', 'token': 'tok1', 'name': 'Alice'})
