...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for perm in permissions_data:
            ...
            obj, created = Permission.objects.get_or_create(codename=perm['codename'], content_type=content_type, defaults={'name': perm['name']})
            if created:
                self.stdout.write(self.style.SUCCESS(f'✅ Criada permissão {obj.codename}'))
            else:
        ...
        for group_data in groups_data:
            ...
            for p in group_data['permissoes']:
                try:
                    perm = Permission.objects.get(codename=p['codename'], content_type__app_label=p['app_label'], content_type__model=p['model'])
                    perms.append(perm)
                except Permission.DoesNotExist:
                    self.stdout.write(self.style.WARNING(f'⚠️ Permissão não encontrada: {p['codename']}'))
            group.permissions.set(perms)
            group.save()
            self.stdout.write(self.style.SUCCESS(f"✅ Grupo '{group.name}' atualizado com {len(perms)} permissões."))
