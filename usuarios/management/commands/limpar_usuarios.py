...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Removendo {total_registros} usuários (não superusers)...')
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ {total_registros} usuários removidos com sucesso!')
            )
            if usernames:
                self.stdout.write(
                    self.style.WARNING(f'Usuários removidos: {", ".join(usernames)}')
                )
