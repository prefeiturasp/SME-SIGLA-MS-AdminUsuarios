...
from usuarios.exceptions import AutenticacaoCredenciaisInvalidasError, AutenticacaoRequisicaoError, AutenticacaoRespostaInvalidaError, AutenticacaoUpstreamError, SmeIntegracaoException
from usuarios.serializers import AlterarEmailSerializer, AlterarSenhaSerializer, BuscarUsuarioEolSerializer, CreateUserSerializer, CriarNovaSenhaSerializer, EsqueciSenhaSerializer, LoginSerializer
...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request: Any) -> Any:
        ...
        try:
            data = AutenticacaoService.autentica(serializer.validated_data['usuario'], serializer.validated_data['senha'])
...
class EsqueciSenhaView(APIView):
    ...
    def post(self, request: Any) -> Any:
        ...
        try:
            EmailService.enviar_email_esqueci_senha(user=user, email=email, nome=nome)
        except Exception:
...
class MeusDadosView(APIView):
    ...
    def get(self, request: Any) -> Any:
        ...
        return Response({'rf': user.username, 'nome_completo': nome_completo, 'email': user.email, 'perfil_acesso': grupos}, status=status.HTTP_200_OK)
...
class BuscarUsuarioEolView(APIView):
    ...
    def post(self, request: Any) -> Any:
        ...
        if User.objects.filter(username=rf).exists():
            return Response({'detail': 'Usuário já cadastrado no SIGLA.'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            info = SmeIntegracaoService.informacao_usuario(rf)
        except SmeIntegracaoException:
            return Response({'detail': 'Usuário não encontrado no EOL.'}, status=status.HTTP_400_BAD_REQUEST)
        except Exception:
            logger.exception('Falha ao consultar EOL para RF: %s', rf)
            return Response({'detail': 'Falha ao consultar o EOL.'}, status=status.HTTP_400_BAD_REQUEST)
...
class CriarUsuarioView(APIView):
    ...
    def post(self, request: Any) -> Any:
        ...
        if User.objects.filter(username=username).exists():
            return Response({'detail': 'Nome de usuário já está cadastrado.'}, status=status.HTTP_409_CONFLICT)
        if User.objects.filter(email__iexact=email).exists():
            return Response({'detail': 'E-mail já está cadastrado.'}, status=status.HTTP_409_CONFLICT)
        partes = nome.strip().split(' ', 1)
        first_name = partes[0]
        last_name = partes[1] if len(partes) > 1 else ''
        user = User.objects.create_user(username=username, email=email, first_name=first_name, last_name=last_name)
        return Response({'detail': 'Usuário criado com sucesso', 'user': user.username}, status=status.HTTP_201_CREATED)
