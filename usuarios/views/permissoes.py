...
from usuarios.serializers.permissoes_serializers import CreateGroupSerializer, CreatePermissionSerializer, GroupSerializer, PermissionSerializer, UpdateGroupPermissionsSerializer, UpdateGroupUsersSerializer, UpdateUsuarioSerializer
...
class GerenciarPermissoesUsuarioView(APIView):
    ...
    def get(self, request: Any) -> Any:
        ...
        return Response({'usuario': user.username, 'nome': nome, 'email': user.email or None, 'grupos': grupos, 'total_permissoes': len(serializer.data), 'permissoes': serializer.data}, status=status.HTTP_200_OK)
...
class UsuariosComGruposView(APIView):
    ...
    def get(self, request: Any) -> Any:
        ...
        for u in qs:
            ...
            data.append({'usuario': u.username, 'nome': nome, 'email': u.email or None, 'is_active': u.is_active, 'grupos': list(u.groups.values_list('name', flat=True))})
    ...
    def patch(self, request: Any) -> Any:
        ...
        nome_resp = (f'{user.first_name} {user.last_name}'.strip() if user.first_name or user.last_name else '') or None
        payload = {'usuario': user.username, 'nome': nome_resp, 'email': user.email or None, 'is_active': user.is_active, 'grupos': list(user.groups.values_list('name', flat=True))}
